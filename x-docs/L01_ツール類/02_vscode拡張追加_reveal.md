ゴールは、ターミナルから指定したファイルまたはフォルダを、VS Codeのエディタでは開かずにExplorer上だけで展開・選択できるようにすること。

以下はlocal起動（localにプロジェクトがある状態）の時に、AIエージェントにそのまま私成功した。

1. VS Code用の小規模なローカル拡張機能を作成する。
2. 拡張機能は外部からURIで呼び出せる構成にする。
3. ターミナル側では `reveal 対象パス` の形式で呼び出せるようにする。
4. `reveal` に渡された相対パスは絶対パスへ変換する。
5. 対象パスはURIとして安全に渡せる形式へ変換する。
6. ターミナルからVS Code拡張機能のURIハンドラを呼び出す。
7. 拡張機能側でURIから対象パスを取得する。
8. 対象ファイルまたはフォルダが実在するか確認する。
9. 実在しない場合は、Explorer操作を行わずエラー表示する。
10. 実在する場合は、対象パスをVS Codeが扱えるURIへ変換する。
11. VS Code内部のExplorer表示コマンドへ対象URIを渡す。
12. Explorerは対象までフォルダ階層を展開し、その項目を選択状態にする。
13. 対象ファイルはエディタで開かないこと。
14. 現在開いているプロジェクトやワークスペースは変更しないこと。
15. ファイルとフォルダの両方を指定可能にすること。
16. Macではシェル設定に `reveal` コマンドを登録して常用できるようにする。
17. 実装後はVS Codeを再読み込みし、相対パス・絶対パス・フォルダ指定で動作確認する。
18. Explorer表示に使用する内部コマンドは将来変更される可能性があるため、その依存箇所は限定して実装する。

--- 以下は ssh に接続した場合（wsl.ubuntuにプロジェクトがある状態）にうまくいかなったときの対応概要
