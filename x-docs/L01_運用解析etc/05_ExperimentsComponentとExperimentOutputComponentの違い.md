```
ExperimentsComponent: Experiment一覧画面（テーブル・フィルタ・選択状態など、experiments.component.ts、NgRxのselectorを大量に使用）
ExperimentOutputComponent: 個別Experiment詳細の出力表示画面（BaseExperimentOutputComponentを継承し、ヘッダー・グラフ設定バー・ナビバーなどをレイアウト）
一覧 vs 単一Experimentの詳細表示、という違い。
```
